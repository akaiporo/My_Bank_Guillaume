...
public class AddUserController extends ControllerBase { 
	...
	/** 
	 * @param event :On va cr�er un �v�nement newCity pour pouvoir rajouter une ville 
	 *  si elle n'existe pas d�j� dans la BDD
	 */
	@FXML
	...
	/**
	 * @param event :On va cr�er un �v�nement handlePostalcode pour pouvoir rajouter un code postal 
	 *  s'l n'existe pas d�j� dans la BDD
	 */
	@FXML 
	...
	/**
	 * 
	 * @param Event : Cr�ation d'un ev�nement avec le bouton ok pour rajouter(set) un owner dans la BDD
	 * En testant � chaque fois les differents param�tres/champs;
	 * Dans l'id�al, il aurait aussi fallu tester l'�galit� des champs pwd et confirm_pwd;
	 * Puis renvoyer une erreur si les deux champs ne sont pas identiques
	 * Apr�s l'ajout on est redirig� vers la page d'authentification
	 */
	@FXML
	private void handleButtonOk(ActionEvent Event) {
		/*
		 * L'objet <CpCity> ("cpcity",créé vide) est relié à un cpcity de la base via une query 
		 */
		...
		/*
		 * L'objet <Address> ("address",créé vide) est retrouvé dans la base ou rempli via les setters en testant chaque champ
		 */
		...
		try {
				address=(Address)q.getSingleResult(); //si adresse déjà existante
		}
		catch (NoResultException err) {
			try {
			...
			address.setLine2(owner_id_address2.getText()); // L'address2 peut �tre vide
			try {
		...
		/*
		 * L'objet <Owner> ("owner",créé vide) est rempli via les setters en testant chaque champ 
		 */
	...
	/**
	 * @param event : L'�v�nement du bouton cancel va permettre  de revenir dans la fen�tre des comptes 
